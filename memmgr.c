...
const int page_fault_count = 0;
const int tlb_hit_count = 0;
int pfc_prev_hit = page_fault_count;                  // new global variable — put at top of file, and initialize to 0
int tlb_prev_hit = tlb_hit_count;                   // new global variable — put at top of file, and initialize to 0
...
unsigned getframe(FILE* fstore, unsigned logic_add, unsigned page,
  ...
  update_tlb(main_mem[i]);
  page_table[current_frame] = page;
  current_frame++;  
    }
   }
 }
...
int get_available_frame(unsigned page) {    // TODO
  // empty queue
  for(int i = 0; i < 128; i++) {page_queue[i] = 0;}
  // queue not full
  if((page_queue[0] = qhead)) {
...
unsigned getframe_fifo(FILE* fstore, unsigned logic_add, unsigned page,
  ...
  // find page location in backing_store
  fstore = fopen("BACKING_STORE.bin", "rb");
  fseek(fstore, current_frame*PAGES, PAGES);
  page_table[current_frame] = page;
  fclose(fstore);
  }
